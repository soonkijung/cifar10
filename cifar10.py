...
class NearestNeighbor:
    ...
    def predict(self, X):
        ...
        # added by skjung for the nearest neighbor image
        Npred = np.zeros(num_test, dtype = 'int') 
        ...
        for i in range(num_test):
            ...
            Npred[i] = min_index # added by skjung
    
        return Ypred, Npred

classifier1NN = NearestNeighbor()
classifier1NN.train(train_images[:100], train_labels[:100])

tests = test_images[:10]
gt = test_labels[:10]
predLabel, nearstIdx = classifier1NN.predict(test_images[:10])
nearest = train_images[findIdx[:]]

import matplotlib.gridspec as gridspec

def plot_testResults(images, labels, nearestImages, predLabels, label_names, columSize):
    ysize = (images.shape[0] + nearestImages.shape[0]) // columSize
    fig = plt.figure(figsize=(15, ysize*1.5))

    for y in range(ysize):
        for x in range(columSize//2):
            gs = gridspec.GridSpec(1, 2, left=(2*x)/columSize+0.05, right=(2*x+2)/columSize, \
                                   bottom=(ysize-y)/ysize+0.05, top=(ysize-y+1)/ysize-0.05, wspace=0.00)
            ax1 = fig.add_subplot(gs[0])
            ax2 = fig.add_subplot(gs[1])
            
            i = y * columSize // 2 + x
            img1 = np.transpose(np.reshape(images[i],(3,32,32)), (1,2,0))
            img2 = np.transpose(np.reshape(nearestImages[i],(3,32,32)), (1,2,0))

            ax1.imshow(img1)
            ax2.imshow(img2)
            
            ax1.set_xlabel(label_names[labels[i]])        
            ax2.set_xlabel(label_names[predLabels[i]])        
...
plot_testResults(tests, gt, nearest, predLabel, label_names, 10)